...
return _joiner(array('cachedir'=>'/static/cache',
					  'tag'=>'link',
					  'type'=>'text/css',
					  'ext'=>'css',
					  'src'=>'href',
...
function _joiner($options,$source) {
	...
	if(is_array($script_array)) {
		...
		if (file_exists($cache_file_dir)) {
			...
			$source = str_replace("@@marker@@","<" . $options['tag'] . " type=\"" . $options['type'] . "\" " .  $options['src'] . "=\"".$cache_http_dir."/$cache_file.$options[ext]\" rel=\"stylesheet\" />",$source);
			return $source;
		}

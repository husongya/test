<?php
class Yummy_Cache_Memcached extends Yummy_Object {
    /**
     * Memcache object
     *
     * @var Memcache
    ...
    public function __construct($options=array()){
        ...
        if(empty($host)){
            throw new Yummy_Exception('memcache server host not specified!');
        }
     ...
     */
    public function get($key,$group=MODULE_NAME){
        if(!is_null($group)){
            $group = MODULE_NAME.':y:'.$group;
     ...
     */
    public function set($key,$value,$group=MODULE_NAME,$ttl=NULL){
     ...
     * @return Doggy_Cache_Provider_Memcached
     */
    public function clear($group=MODULE_NAME){
        if(is_null($group)){
            $this->memcache->flush();
        }else{
            $group = MODULE_NAME.':y:'.$group;
     ...
     * @return Doggy_Cache_Provider_Memcached
     */
    public function remove($key,$group=MODULE_NAME){
        if (!is_null($group)) {
            $group = MODULE_NAME.':y:'.$group;
            $key = $this->_parseGroupDataKey($group,$key);
        }
